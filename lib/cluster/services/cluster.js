...
module.exports = function(context, server) {
    ...
    function verifyAssets(assets) {
        var msgObj = _.assign({message: 'jobs_service:verify_assets'}, assets);
    ...
    messaging.register('assets:preloaded', function(msg) {
    ...
    function notifyNode(node_id, msg, msgData) {
        return new Promise(function(resolve, reject) {
            ...
            setTimeout(function() {
                // remove listener to prevent memory leaks
                events.removeAllListeners(msgData._msgID);
                reject(`timeout error while communicating with node: ${node_id}, msg: ${msg}, data: ${JSON.stringify(msgData)}`);
            }, configTimeout);
        });
    }
    ...
    function removeWorkers(res, ex_id, workerNum) {
        ...
        Promise.all(results)
            ...
            .catch(function(err) {
                var errMsg = parseError(err);
                logger.error(`Error while releasing workers from job ${ex_id}, error: ${errMsg}`);
